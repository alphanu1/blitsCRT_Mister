...
# Vendor stubs for simulation.
#
# rtl/pll_pix.v and rtl/pll_reconfig.v are generated by Quartus and committed,
# because a bitstream build needs them -- but a fresh checkout that has not run
# Quartus does not have them, and iverilog needs *something* to elaborate
# against. So the PLL stubs live in their own file and are added only when the
# real ones are absent. Including both is a duplicate-module error:
#
#   sim/vendor_stubs.v:70: error: 'pll_pix' has already been declared
#
# which only appears once the generated files exist, so it shows up on someone
# else's machine or in CI rather than on the one that has been building all
# along.
...
lint: check-decl check-clk
	$(need_iverilog)
	$(IV) -o /dev/null -s lint_onchip rtl/*.v $(VENDOR_STUBS) sim/lint_onchip.v && echo "top elaborates clean (ONCHIP)"
	$(IV) -o /dev/null -s lint_ddr3 rtl/*.v $(VENDOR_STUBS) sim/lint_ddr3.v && echo "top elaborates clean (DDR3, stubbed f2sdram)"
