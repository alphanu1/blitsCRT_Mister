...
# Two modes, and bank 2 left blank.
#
# There were three. 640x480p60 at 31.5 kHz was a diagnostic for a monitor that
# will not take 15 kHz; it was removed from mode_table.v since this drives CRTs.
# Its bank stays in the ROM rather than renumbering, because bank 3 is the
# no-daemon banner and the overlay indexes {bank, row} directly.
BANKS = ['640x240p60', '640x480i60', None]
...
def main():
    ...
    for bank, mode_name in enumerate(BANKS):
        if mode_name is None:
            continue                    # bank 2, retired; left as spaces
    ...
    with open(out, 'w') as f:
        f.write("// overlay character buffer, 2 mode banks (2 retired) + 1 fabric banner, %d rows each\n"
                % ROWS_PER_BANK)
